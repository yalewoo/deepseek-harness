...
Session 行内的 Rename 操作打开一个以该行显示标题预填的对话框；确认未修改的标题是有意允许的——这正是把当前自动标题钉住、不再被重新生成覆盖的手势。Archive 不经确认对话框直接提交，归档集合回声落地后，该行从所有分组视图中消失。进入编辑会话模式后，可以勾选单个 Session，也可以全选所有未归档且非空白的 Session；选择范围跨越折叠分组和两种分组模式，失败请求会保持选中以便重试。退出编辑模式会清空选择，并恢复普通的行导航、菜单和拖拽。已归档行仍可打开查看，并支持恢复、单选或全选，以及确认后永久删除。删除失败的 Session 会保持选中，并显示第一条运行时原因以便诊断后重试。永久删除会先停止活动 Agent，再移除完整 Session 产物目录和全部 Workspace/归档引用。Fork 在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话。Workspace 行内的 Delete 操作会打开确认框，说明保留边界；成功后该分组被移除，其 Session 则留在 Ungrouped 下。
...
这些限制定义搜索深度、分组状态呈现与选取载体；它们是当前包约束。
