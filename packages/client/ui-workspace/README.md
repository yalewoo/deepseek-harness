...
The Session row's Rename action opens a dialog prefilled with the row's display title; confirming an unchanged title is deliberately allowed — it pins the current automatic title against regeneration. Archive commits without a confirmation dialog and moves the row from grouping surfaces into the header's Archived view. Edit sessions mode exposes checkboxes for individual Sessions and a select-all action covering every unarchived, non-blank Session; selection spans folded groups and either grouping mode, and failed requests remain selected for retry. Leaving edit mode clears the selection and restores normal row navigation, menus, and dragging. Archived rows remain openable and can be restored, selected individually or all at once, and permanently deleted after confirmation. Failed deletions remain selected and report the first runtime reason for retry diagnostics. Permanent deletion stops a live Agent before removing the complete Session artifact directory and every Workspace/archive reference. Fork forks at the source's last completed turn, increments the inherited persisted title on the client, and then opens the child. Workspace Delete opens a confirmation that states the retention boundary; success removes the group while its Sessions remain under Ungrouped.
...
These limits define the search depth, grouped status presentation, and the picking carrier; they are current package constraints.
