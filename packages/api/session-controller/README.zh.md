...
fork 会先把新的普通 Agent 交由 Session Controller 保留，再将其暴露给其他生命周期操作。在 turn 之前截断的 fork 会清除从该边界继承的 pending inbox 状态。重放型 fork 随后会先完成目标 Workspace 挂接，再接纳选中的用户消息；挂接失败时直接返回，不唤醒 child。
